...
simular.proceso <- function(data,
    ...
    verificacion <-
        function(p = numeric(0),
            ...
            list(
                orden_no_estacional = c(length(p), length(d), length(q)),
                orden_estacional = c(length(P), length(D), length(Q)),
                p = p,
                d = d,
                q = q,
                P = P,
                ...
                cumple = IdentInvertQ(
                    ...
                    phiseas = P,
                    thetaseas = Q,
                    period = ifelse(sum(estacional)==0, 0, tempo)
                )
            )
    ...
    while(invertible_estacionario_identificable$cumple == FALSE) {
        ...
        #list(coeficientes, invertible_estacionario_identificable)
    }
