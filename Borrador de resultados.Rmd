...
escenarios <- expand.grid(p=2:5, d=0:1, q=2:5,P=2:5, D=0:1, Q=2:5)
no_estacional <- escenarios %>% 
    select(p:q)
no_estacional <- no_estacional[rowSums(no_estacional)>0,] %>% 
    unique
...
prova <- op.arima(arima_process = c(3,1,3,3,1,3),
                  seasonal_periodicity = 12,
                  time_serie = resultados_estacional_bajo[[2]]$datos,
                  horiz=50,
                  prop = .8,
                  reg=NULL,
                  )

resultados_estacional_bajo

Arima(resultados_estacional_bajo[[2]]$datos, c(1,1,1), c(1,1,1), method = "ML")
```
...
# INEC: Time difference of 1.204223 hours
